...
class ControlLDM(LatentDiffusion):
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        log["conditioning"] = log_txt_as_img((512, 512), batch[self.cond_stage_key], size=32)
...
from ldm.modules.encoders.modules import ClipImageEmbedder

class ControlNetClip(ControlNet):
    def __init__(self, image_size, in_channels, model_channels, hint_channels, num_res_blocks, attention_resolutions, dropout=0, channel_mult=(1, 2, 4, 8), conv_resample=True, dims=2, use_checkpoint=False, use_fp16=False, num_heads=-1, num_head_channels=-1, num_heads_upsample=-1, use_scale_shift_norm=False, resblock_updown=False, use_new_attention_order=False, use_spatial_transformer=False, transformer_depth=1, context_dim=None, n_embed=None, legacy=True, disable_self_attentions=None, num_attention_blocks=None, disable_middle_self_attn=False, use_linear_in_transformer=False):
        ...
        self.hint_encoder = ClipImageEmbedder(freeze=True)
        self.input_hint_block = nn.Sequential(
            Upsample(channels=768, use_conv=True, dims=2, out_channels=768),
            Upsample(channels=768, use_conv=True, dims=2, out_channels=320),
        )
    
    def forward(self, x, hint, timesteps, context, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
        hint = self.hint_encoder(hint)
        hint = rearrange(hint, 'b (h w) c ->  b c h w', h=16)
...
class ControlLDMPlus(ControlLDM):
    def __init__(self, control_stage_config, control_key, only_mid_control, hint_stage_config, sd_locked=True, *args, **kwargs):
        super().__init__(control_stage_config, control_key, only_mid_control, sd_locked, *args, **kwargs)
        self.hint_stage_model = instantiate_from_config(hint_stage_config)

    def get_input(self, batch, k, bs=None, *args, **kwargs):
        x, c = LatentDiffusion.get_input(self, batch, self.first_stage_key, *args, **kwargs)
        # control = batch[self.control_key]
    ...
    def configure_optimizers(self):
        lr = self.learning_rate
        params = list(self.control_model.parameters())
        params += list(self.hint_stage_model.parameters())
        if not self.sd_locked:
            params += list(self.model.diffusion_model.output_blocks.parameters())
            params += list(self.model.diffusion_model.out.parameters())
        opt = torch.optim.AdamW(params, lr=lr)
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        n_row = min(z.shape[0], n_row)
        log["reconstruction"] = self.decode_first_stage(z)
        log["control_img"] = c_cat[:,:3]
        log["control_flow"] = c_cat[:,3:]
        # log["conditioning"] = log_txt_as_img((512, 512), batch[self.cond_stage_key], size=32)
...
class ControlUnclipLDM(LatentDiffusion):
    ...
    def get_unconditional_conditioning(self, N, label=''):
        if hasattr(self.cond_stage_model, 'get_unconditional_conditioning'):
            return self.cond_stage_model.get_unconditional_conditioning(N)
        return self.cond_stage_model([label] * N)
